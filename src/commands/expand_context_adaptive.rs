...
pub async fn run(
    ...
    let doc_id = doc_table.doc_id(&passage_id)
    ...
    loop {
        let node = catalog.get_node(node_id).ok_or_else(|| anyhow!("bad node_id"))?;
        ...
        match node.parent_id {
            Some(parent) => {
                node_id = parent;
                climbed += 1;
                let parent_node = catalog.get_node(node_id).ok_or_else(|| anyhow!("bad parent_id"))?;
    ...
    let selected = catalog.get_node(node_id).ok_or_else(|| anyhow!("no selected node"))?;
    let first = selected.first_doc_id.ok_or_else(|| anyhow!("node has no doc range"))?;
    let last  = selected.last_doc_id.ok_or_else(|| anyhow!("node has no doc range"))?;
