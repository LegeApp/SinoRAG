...
const DOCS: &[ToolDoc] = &[
    ...
    ToolDoc { name: "report-build", purpose: "Assemble a markdown report from finished adjudication and graph files.", use_when: "Use when producing a dossier/report artifact from completed, written-up evidence. To render raw markdown directly without an adjudication, use pdf-build's input_markdown instead.", notes: "Writes output files." },
    ...
    ToolDoc { name: "works", purpose: "List works from the catalog, or look up one work's metadata directly by work_id.", use_when: "Use to identify work IDs and filter corpus areas by metadata, or to fetch a single work's title/author/period/canon/traditions when you already have its ID — a lighter alternative to source-read for metadata-only lookups.", notes: "Requires catalog.index." },
    ...
    ToolDoc { name: "report-from-evidence", purpose: "Validate adjudication, build graph, and build report in one workflow.", use_when: "Run this after your investigation is done and you've written up an adjudication — for reports straight from prose without one, use pdf-build's input_markdown instead.", notes: "Writes output files and respects readonly/output-root safety." },
];
