...
pub async fn build(brief: Brief, options: BuildOptions) -> Result<PathBuf> {
    ...
    eprintln!(
        "Recipe      : {} ({} steps, full_work_threshold={})",
        recipe.name,
        recipe.steps.len(),
    ...
    eprintln!(
        "       {} passages written ({} distinct works)",
        stats.passages_written,
        stats.works_seen.len()
    );

    eprintln!(
        "[3/6 contexts] expanding contexts (±{}/±{})...",
    ...
    let contexts_written = assemble::write_contexts(
        ...
        &pack,
        &stats.cited,
        recipe.context_before,
        recipe.context_after,
    )
    .await?;
    eprintln!("       {} contexts written", contexts_written);

    eprintln!(
        "[4/6 documents] full-text for works with >= {} cited passages...",
        recipe.full_work_threshold
    );
