...
fn try_emit_passage(
    ...
    let wid = item
        ...
        .to_string();
    if !wid.is_empty() {
        *works.entry(wid.clone()).or_insert(0) += 1;
    }

    cited.push(CitedPassage {
        passage_id: pid.to_string(),
        source_work_id: wid,
        source_rel_path: item
            .get("source_rel_path")
            .and_then(Value::as_str)
            .unwrap_or("")
            .to_string(),
    ...
    std::fs::write(
        passages_dir.join(format!("{safe}.md")),
        render_passage_md(item),
    )?;
    *written += 1;
    Ok(())
}
...
pub async fn write_documents(
    ...
    let heavy: Vec<&String> = works_seen
        .iter()
...
pub fn write_pre_diagrams(
    ...
    let evidence_items: Vec<Value> = cited
        .iter()
        .map(|c| {
    ...
    std::fs::write(
        diagrams_dir.join("evidence.json"),
        serde_json::to_vec_pretty(&evidence)?,
    )?;
    std::fs::write(
        diagrams_dir.join("timeline.json"),
        serde_json::to_vec_pretty(&timeline)?,
    )?;
    std::fs::write(
        diagrams_dir.join("lineage.json"),
...
pub fn write_readme(packet_root: &Path, brief: &Brief, recipe: &Recipe) -> Result<()> {
    ...
    s.push_str(
        "| `manifest.json` | Provenance: pack id, fingerprint, brief, recipe, every tool call. |\n",
    );
    ...
    s.push_str(
        "2. Read `tools/*.json` for structured findings (each file is one tool's raw output).\n",
    );
...
pub fn write_index_jsonl(packet_root: &Path) -> Result<()> {
    ...
    for entry in walkdir::WalkDir::new(packet_root)
        .into_iter()
        .filter_map(Result::ok)
    {
        ...
        writeln!(
            w,
            "{}",
            json!({ "path": rel.to_string_lossy(), "bytes": bytes })
        )?;
    }
    Ok(())
}
